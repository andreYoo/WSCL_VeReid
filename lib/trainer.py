...
from .loss import WSCL_SDA_Loss
...
class Trainer(object):
    def __init__(self, cfg, model, memory,use_dram=False):
        ...
        self.criterion = WSCL_SDA_Loss(temperature=cfg.WSCL.TEMP, base_temperature=cfg.WSCL.BTEMP,contrast_mode=cfg.WSCL.MODE, gz_ratio=self.gz).to(self.device)
    ...
    def train(self, epoch, data_loader, optimizer,writer,gi=True, print_freq=1):
        ...
        for i, inputs in enumerate(data_loader):
            ...
            if epoch > 5:
                ...
                cl_loss,_hard_pos = self.criterion(self.memory,logits,camid,hard_pos=None,trackids=tid,type='WSCL')

                pc_loss,_t1,_t2 = self.criterion(self.memory,logits,camid,hard_pos=None,trackids=tid,type='SDA')
                loss = cl_loss +self.ldb*pc_loss
                ...
                loss,_ = self.criterion(self.memory,logits,camid,trackids=tid,type='WSCL')
